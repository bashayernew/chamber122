 ...
 * @returns {Promise<{data: Array, error: Error|null}>}
 */
export async function fetchEventsWithBusiness({ scope = 'public', kind = 'all', upcomingOnly = true } = {}) {
  try {
    console.debug(`[events] loading with business info scope=${scope}, kind=${kind}, upcomingOnly=${upcomingOnly}`);
    ...
    if (error) {
      ...
      // Fallback: try without business join if RLS is blocking
      console.log('[events] trying fallback query without business join...');
      ...
      if (scope === 'public') {
        fallbackQuery.eq('status', 'published').eq('is_published', true);
        if (upcomingOnly) {
          fallbackQuery.gte('start_at', new Date().toISOString());
        }
      }
      ...
      const transformedFallbackData = (fallbackData || []).map(event => ({
        ...event,
      ...
      console.debug(`[events] loaded ${transformedFallbackData.length} events (fallback mode)`);
      return { data: transformedFallbackData, error: null };
    }
